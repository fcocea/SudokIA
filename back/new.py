...
def get_unassigned_locations(board):
    if not all(len(row) == 9 for row in board):  # Check if all rows have length 9
        raise ValueError("Invalid Sudoku puzzle: All rows must have 9 elements.")
...
def mrv_heuristic(board):
    ...
    return min(unassigned, key=lambda x: len(get_possible_values(board, *x)))

def lcv_heuristic(board, row, col):
    possible_values = get_possible_values(board, row, col)
    return sorted(possible_values, key=lambda val: count_constraints(board, row, col, val))
...
def backtrack(board, heuristic=None):  # Make heuristic optional
    if is_complete(board):
        return board
    
    if heuristic:  # Check if a heuristic is provided
        next_cell = heuristic(board)
        if next_cell is None:
            return None
    else:  # No heuristic, select the first empty cell
        unassigned = get_unassigned_locations(board)
        if not unassigned:
            return None
        next_cell = unassigned[0]  # Select the first unassigned cell

    row, col = next_cell

    possible_values = (
        lcv_heuristic(board, row, col) if heuristic == lcv_heuristic else get_possible_values(board, row, col)
    )
...
def main():
    while True:
        try:
            num_puzzles_to_test = int(input("Enter the number of puzzles to test (or 0 to test all): "))
    ...
    total_times = {"normal": 0, "mrv": 0, "lcv": 0}
    ...
    with open('../data/test.csv', newline='') as csvfile:
        ...
        for quiz_str, solution_str in reader:
            if num_puzzles_to_test == 0 or num_puzzles_solved < num_puzzles_to_test:
                ...
                quiz = [list(map(int, quiz_str[i:i+9])) for i in range(0, 81, 9)]
                solution = [list(map(int, solution_str[i:i+9])) for i in range(0, 81, 9)]
